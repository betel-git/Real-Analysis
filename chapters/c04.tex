...
\begin{definition}
    Пусть $\mu_0: \mathcal{P} \to [0, + \infty]$ — счётно-аддитивная мера на полукольце $\mathcal{P}$ подмножеств множества $X$. Для любого $E \subset X$ определим его \textit{внешнюю меру по Каратеодори}:
    \[\mu_0^*(E) := \inf \left\{\sum\limits_{j=1}^{\infty} \mu_0(P_j) \ : \ \{P_j\} \subset \mathcal{P}, \ E \subset \bigcup\limits_{j=1}^{\infty}P_j\right\},\]
...
\begin{proof}
    ...
    Покажем, что $\mu_0^*$ — счётно-полуаддитивная мера на $2^X$ ($\mu_0^*(\emptyset)=0$ очевидно из только что доказанного).
    Пусть $E \subset \bigcup\limits_{i=1}^{\infty} E_i$, где $E \in 2^X$, $\{E_i\} \subset 2^X$.
    Если существует $i \in \N$: $\mu_0^*(E_i) = + \infty$, то $\mu_0^*(E) \leqslant \sum\limits_{i=1}^{\infty} \mu_0^*(E_i)$ очевидно, поэтому далее считаем, что $\mu_0^*(E_i) < + \infty$ для любого $i \in \N$.
...
\begin{theorem}
    ...
    \begin{enumerate}
        ...
        \item $\mu_0^*$ — счётно-аддитивная мера на $\mathfrak{M}_{\mu_0^*}$;
...
\begin{proof}
    ...
    \[\forall E \subset X \ \forall P \in \mathcal{P} \quad \mu_0^*(E) \geqslant \mu_0^*(P \cap E) + \mu_0^*(E \setminus P)\]
...
\begin{theorem}
    ...
    Тогда $\mu$ счётно-аддитивна тогда и только тогда, когда $\mu$ непрерывна снизу по включению, то есть если
...
\begin{definition}
    ...
    и при этом 
    \[\mu(P_n) < + \infty \quad \forall n \in \N.\]
\end{definition} 
...
\begin{definition}
    ...
    Тогда $\mu$ называется \textit{полной мерой}, если $\forall E \in \mathfrak{M}: \ \mu(E) = 0$ и $\forall E' \subset E$ выполнено $E' \in \mathfrak{M}$.
\end{definition} 

\begin{theorem}[О единственности продолжения меры]
    Пусть $\mu^*$ — продолжение счётно-аддитивной меры $\mu$ с $\mathcal{P}$ на $\sigma$-алгебру $\mathfrak{M}_{\mu^*}$, $\nu$ — продолжение $\mu$ с $\mathcal{P}$ на $\sigma$-алгебру $\mathfrak{M}_{\nu}$.
    Тогда:
    \begin{enumerate}
...
\begin{proof}\tab
    \begin{enumerate}
        ...
        \[\underbrace{\sum\limits_{j=1}^{\infty}\mu^*(A \cap P_j)}_{\mu^*(A) \leqslant} = \sum\limits_{j=1}^{\infty} \nu(A \cap P_j) = (*)\]
...
\begin{lemma}
    Пусть $\mu^*$ — внешняя мера, построенная по счётно-аддитивной мере $\mu$ на полукольце $\mathcal{P}$.
...
\begin{proof}
    ...
    Тогда
    \[C_N = \bigcap\limits_{n=1}^{N} \bigcup\limits_{j=1}^{\infty} P_{n,j} \supset E \ \forall N \in \N \implies 
    C = \bigcap\limits_{n=1}^{\infty} \bigcup\limits_{j=1}^{\infty} P_{n,j} \supset E,\]
    значит, $C \in \mathfrak{M}_{\mu^*}$.
    Тогда 
    \[\mu^*(C) \leqslant \mu^*(C_N) \leqslant \mu^*\left(\bigcup\limits_{j=1}^{\infty} P_{N,j}\right) \leqslant (1) \leqslant \sum\limits_{j=1}^{\infty} \mu^*(P_{N,j}) \leqslant \mu^*(E) + \frac{1}{2^N}.\]
...
\begin{proof}
    ...
    Поскольку $\mu^*$ полна и $e \in \mathfrak{M}_{\mu^*}$, то
