...
\end{remark}
\begin{statement}
    Пусть $\widetilde{h}(x) \leqslant h(x)$ на $E$.
...
\begin{proof}
    ...
    Пусть
    \begin{equation}
        0 \leqslant g(x) \leqslant f(x) \ \forall x \in E \label{eq:9.2}
    \end{equation}
...
\begin{proof}
    \[A \subset B
    \implies
    \chi_A f(x) \leqslant \chi_B f(x) \ \forall x \in X,\]
...
\begin{proof}
    ...
    Применим к $\{g_n\}$ теорему Беппо-Леви:
    ...
    \begin{multline*}
        ...
        \leqslant\\\leqslant
        \inf\limits_{k \geqslant n} \int_X f_k(x) d\mu(x)
        \leqslant
        \lowlim\limits_{n\to \infty} \int_X f_n(x) d\mu(x).
    \end{multline*} 
\end{proof} 
...
\begin{statement}
    Если $f_1, \dots, f_N \widetilde{L}_1(E, \mu)$ и $\alpha_1, \dots, \alpha_N \in \R$.
    Тогда
