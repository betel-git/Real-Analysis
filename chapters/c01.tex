...
\begin{lemma}[О дизъюнктном объединении]
    ...
    \[\bigcup\limits_{n=1}^{\infty} A_n = \bigsqcup\limits_{n=1}^{\infty} \left(A_n \setminus \bigcup\limits_{n' = 0}^{n - 1} A_{n'}\right).\]
\end{lemma} 
\begin{proof}
    ...
    $$x \in A_{m(x)} \setminus \bigcup_{n' = 0}^{m(x) - 1} A_{n'} \Longrightarrow x \in \bigsqcup\limits_{n=1}^\infty \left(A_n \setminus \bigcup\limits_{n' = 0}^{n - 1} A_{n'} \right).$$
\end{proof} 
...
\begin{definition}
    ...
    \begin{enumerate}
        ...
        \item $A, B \in \mathcal{R} \Longrightarrow A \cup B \in \mathcal{R},$
        \item $A, B \in \mathcal{R} \Longrightarrow A \setminus B \in \mathcal{R}.$
    \end{enumerate}
\end{definition} 

\begin{remark}
    Имея замкнутость относительно разности, можно получить первые два условия:
...
\begin{remark}
    ...
    \begin{enumerate}
        \item $\emptyset \in \mathcal{A} \quad X \in \mathcal{A}$;
        \item $A \bigtriangleup B \in \mathcal{A} \quad \forall A, B \in \mathcal{A}$;
        \item $A \cap B \in \mathcal{A} \quad \forall A, B \in \mathcal{A}$.
    \end{enumerate}
\end{remark}
...
\begin{definition}
    ...
    \begin{enumerate}
        ...
        \item $A \cap B \in \mathcal{P} \quad \forall A, B \in \mathcal{P}$;
...
\begin{theorem}[Дизъюнктное представление в полукольце]\label{th:1.2}
    ...
    \begin{enumerate}
        ...
        \item Тогда $\forall \{P_i\}_{i = 1}^N \subset \mathcal{P}$, $N \in \N$ существует дизъюнктный конечный набор $\{Q_j\}_{j=1}^L \subset \mathcal{P}$, $L \in \N$: 
...
\begin{enumerate}
    ...
    \[P \setminus \bigcup\limits_{i = 1}^{N' + 1} P_i = P \setminus \left(\bigcup\limits_{i = 1}^{N'} P_i \cup P_{N' + 1}\right) = \left( P \setminus \bigcup\limits_{i=1}^{N'} P_i \right) \cap (P \setminus P_{N' + 1}),\]
...
\begin{proof}
    ...
    \noindent Шаг индукции:
    Выделим из $P_n$ часть, которая не пересекается с предыдущими $\{P_i\}_{i=1}^{n-1}$:
    \[P_n \setminus \bigcup\limits_{k=1}^{n-1} P_k = \bigsqcup\limits_{j=1}^{m_n} Q_{n,j}, \ \forall n \in \N.\]
    — по теореме о дизъюнктном представлении.
    Все $Q_{n,j}$ не пересекаются с ранее построенными в предположении множествами, поскольку они лежат в дополнении, следовательно, получаем необходимое дизъюнктное объединение.
\end{proof} 

\begin{corollary}
    Любое непустое открытое множество $\Omega$ в $\R^n$ можно реализовать в виде не более чем счётного объединения ячеек вида
    \[[a,b) = \prod_{i=1}^n [a_i, b_i), \ a_i, b_i \in \Q.\]
...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        \item $A_1 \times A_2 \in \mathcal{P}_1 \otimes \mathcal{P}_2$, $B_1 \times B_2 \in \mathcal{P}_1 \otimes \mathcal{P}_2$
        ...
        \item Пусть $A \times B \in \mathcal{P}_1 \otimes \mathcal{P}_2$, $C \times D \in \mathcal{P}_1 \otimes \mathcal{P}_2$.
        Определим $C_0 := A \cap C$, $D_0 := B \cap D$. Тогда поскольку $\mathcal{P}_1$ и $\mathcal{P}_2$ — полукольца, то
        \[A \setminus C_0 = \bigsqcup\limits_{j=1}^{N_1} C_j \implies A = C_0 \sqcup \bigsqcup\limits_{j=1}^{N_1} C_j, \quad C_j \in \mathcal{P}_1,\]
        \[B \setminus D_0 = \bigsqcup\limits_{i=1}^{N_2} D_i \implies B = D_0 \sqcup \bigsqcup\limits_{i=1}^{N_2} D_i, \quad D_i \in \mathcal{P}_2.\]
