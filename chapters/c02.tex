...
\begin{definition}
    ...
    \begin{enumerate}
        \item $\mu(\emptyset) = 0,$
        \item $A \in \mathcal{P}$, $A = \bigsqcup\limits_{i=1}^N A_i$, $\{A_i\} \subset \mathcal{P}$, то $\mu(A) = \sum\limits_{i=1}^N \mu(A_i)$.
    \end{enumerate}
    Если же выполнено условие
    \begin{enumerate}
        \item[$2'.$] $A \in \mathcal{P}$, $A = \bigsqcup\limits_{i=1}^{\infty} A_i$, $\{A_i\} \subset \mathcal{P}$, то $\mu(A) = \sum\limits_{i=1}^{\infty} \mu(A_i)$,
    \end{enumerate}
    то говорят, что $\mu$ — \textit{счётно-аддитивная мера} (\textit{$\sigma$-аддитивная мера} или просто \textit{мера}) на $\mathcal{P}$.
\end{definition} 
...
\begin{proof}\tab
    \begin{enumerate}
        ...
        так как $\sum\limits_{l=1}^{L}\mu(Q_l) \geqslant 0$.
        \item $C_j \cap B =: B_j$, где $C_j \cap B \in \mathcal{P}$ $\forall j \in \{1, \dots, M\}$. Тогда
        \[B = \bigcup\limits_{j=1}^{M} B_j = \bigsqcup\limits_{l=1}^{L}B_l^{'},\]
...
\begin{proof}
    ...
    \begin{multline*}
        \mu(A \times B) := \mu_1(A) \cdot \mu_2(B) = \left(\sum\limits_{i=1}^{N}\mu_1(P_i)\right) \cdot \left(\sum\limits_{j=1}^{M}\mu_2(Q_j)\right) =\\= \sum\limits_{i=1}^{N} \sum\limits_{j=1}^{M} \mu_1(P_i) \mu_2(Q_j) = \sum\limits_{i=1}^{N} \sum\limits_{j=1}^{M} \mu(P_i \times Q_j).
    \end{multline*}
...
\begin{proof}\tab
    \begin{itemize}
        \item[$\underline{2 \Longrightarrow 1}$]
    ...
    \noindent Поскольку $\mu$ конечно-аддитивна, в силу продвинутой монотонности $\forall N \in \N$ $\mu(A) \geqslant \sum\limits_{j=1}^{N}\mu(A_j)$, то возьмём супремум по $N$ и получим $\mu(A) = \sum\limits_{j=1}^{\infty} \mu(A_j)$.
        \item[$\underline{1 \Longrightarrow 2}$]
